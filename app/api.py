...
async def add_post(post: Post, db: Session = Depends(get_db), user: SystemUser = Depends(get_current_user)) -> dict:
    return crud.create_post(db, post=post)


@app.put("/posts/{post_id}", tags=["api"])
async def update_post(post_id: int, post: PostBase, db: Session = Depends(get_db), user: SystemUser = Depends(get_current_user)) -> dict:
    return crud.update_or_create_post(db, post_id=post_id, post=post)


@app.delete("/posts/delete/{post_id}", status_code=204, tags=["api"])
async def delete_post(post_id: int, db: Session = Depends(get_db), user: SystemUser = Depends(get_current_user)) -> dict:
